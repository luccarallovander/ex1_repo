...
A day is considered "hot" (coded 1) if the temperature (`Temp` column) is above 80°F, 
or not (coded 0).

```{r}
add_hot_day_column <- function(data) {
  ...
  if ("Temp" %in% names(data)) {
    ...
    data$Hot_Day <- ifelse(data$Temp > temp_threshold, 1, 0)
    return(data)
  } else {
    stop("The data does not have a 'Temp' column.")
  }
